...
p2 <- p2 + coord_flip() + theme(legend.position = "none", axis.title.y = element_blank(), title = element_text(colour = "white")) + scale_x_discrete(limits = rev(levels(data.vis$Continent)))
p3 <- plot_grid(p, p2)
p3
```
...
```{r}
ggviolin(data.vis, x = "Continent", y = "total", fill = "Continent", palette = witness.me, add = c("boxplot")) + geom_jitter(width = .1, alpha = .2) + coord_flip() + theme(legend.position = "none")
```
