...
### To summarize `plotly`
__Pros__: A strong tool for interactive ploting especially if you want to use Shiny. Also has the ggplotly option taht converts all ggplots to interactive ones.
...
The basic  `ggvis()` plots are static just as ggplot, but perhaps need less writing to produce.
...
### Basic example
Lets put a random point on a defult map
...
We can change the base map we are using to either ome from the internet or a shapefile we have.
We will do this using `addProviderTiles()` that host many free third-party maps.
...
```{r}
# create a string with the adress and the link to the location
...
# plot a defult map
leaflet() %>% addTiles() %>%
  #add popup gps coordinates, contatnt variable allows the popup to appear
...
Lets put popups on the center of all our countries with their names
...
Labels will make the information in the popup markers appear when you hoover over them
...
Lets use only three countries so that it will be a bit more useful. In this case we'll make the lables to be constant using `noHide = T` in the `labelOptions`
...
leaflet(df2) %>% addTiles() %>%
  addMarkers(~x[1], ~y[1], label = ~htmlEscape(CNTRY_NAME[1]),
             #this is a use of CSS programing language to change the parameters of the label
...
Lets plot color only few countries from our polygon
...
leaflet(data_vis.sp_sub) %>%
  ...
  addPolygons(color = "#444444", weight = 1, smoothFactor = 0.5,
    ...
    #fill color says based on what to color and the (SQKM_CNTRY) makes sure that it is indeed colord
...
```{r}
# We'll use the data from the sapefile we created before
...
Choropleths refers to maps in which the polygons are colored based on a continious or a discrete variable. This is similar to what we did with `ggplot2`

Lets start with a basic one, color the countries based on spirit consumption
```{r}
# create the color palet with viridis let it decide the bins by itself
...
Lets add an option of looking which country we are standing with the mouse
